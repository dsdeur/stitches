...
const split = (fn: (...args: string[]) => Record<string, string>) => (data: string) => fn(...(typeof data === 'string' ? String(data).split(splitBySpace) : [data]))

export const toPolyfilledValue: Record<string, (d: string) => Record<string, string>> = {
	// prefixed properties
