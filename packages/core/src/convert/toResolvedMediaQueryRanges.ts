...
export const toResolvedMediaQueryRanges = (media: string): string =>
	media.replace(/\(\s*([\w-]+)\s*(=|<|<=|>|>=)\s*([\w-]+)\s*(?:(<|<=|>|>=)\s*([\w-]+)\s*)?\)/g, (__: string, p1: string, o1: string, p2: string, o2: string, p3: string) => {
		...
		return (
			...
			name +
			':' +
			(o1[0] !== '=' && o1.length === 1 ? value.replace(mqunit, (_: string, v: string, u: string) => Number(v) + shift * (o1 === '>' ? 1 : -1) + u) : value) +
			(o2 ? ') and (' + ((o2[0] === '>' ? 'min-' : 'max-') + name + ':' + (o2.length === 1 ? p3.replace(mqunit, (_: string, v: string, u: string) => Number(v) + shift * (o2 === '>' ? -1 : 1) + u) : p3)) : '') +
			')'
		)
	})
