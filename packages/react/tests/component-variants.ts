...
describe('Variants', () => {
	...
	test('Renders a component with 1 matching variant', () => {
		const { styled, toString } = createStitches()
		const component = styled('div', componentConfig)
		const expression1 = render(component, { size: 'small' })
		...
		const expression2 = render(component, { color: 'blue' })
	...
	test('Renders a component with 2 matching variants', () => {
		const { styled, toString } = createStitches()
		const component = styled('div', componentConfig)
		const expression = render(component, { size: 'small', level: 1 })
	...
	test('Renders a component with a 2 matching variants and 1 matching compound', () => {
		const { styled, toString } = createStitches()
		const component = styled('div', componentConfig)
		const expression = render(component, { size: 'small', color: 'blue' })
...
describe('Variants with defaults', () => {
	...
	test('Renders a component with the default variant explicitly applied', () => {
		const { styled, toString } = createStitches()
		const component = styled('div', componentConfig)
		const expression = render(component, { size: 'small' })
	...
	test('Renders a component with the non-default variant explicitly applied', () => {
		const { styled, toString } = createStitches()
		const component = styled('div', componentConfig)
		const expression = render(component, { size: 'large' })
	...
	test('Renders a component with the default variant applied and a different variant explicitly applied', () => {
		const { styled, toString } = createStitches()
		const component = styled('div', componentConfig)
		const expression = render(component, { level: 1 })
	...
	test('Renders a component with the default variant applied, a different variant explicitly applied, and a compound applied', () => {
		const { styled, toString } = createStitches()
		const component = styled('div', componentConfig)
		const expression = render(component, { color: 'blue' })
...
describe('Conditional variants', () => {
	...
	test('Renders a component with no variant applied', () => {
		...
		expect(render(component).props.className).toBe(componentClassName)
		expect(toString()).toBe('')
	})
	...
	test('Renders a component with one variant applied', () => {
		...
		expect(render(component, { size: 'small' }).props.className).toBe([componentClassName, componentSmallClassName].join(' '))
	...
	test('Renders a component with one conditional variant on one breakpoint applied', () => {
		...
		expect(render(component, { size: { '@bp1': 'small' } }).props.className).toBe([componentClassName, componentSmallBp1ClassName].join(' '))
	...
	test('Renders a component with one conditional variant on two breakpoints applied', () => {
		...
		expect(render(component, { size: { '@bp1': 'small', '@bp2': 'large' } }).props.className).toBe([componentClassName, componentSmallBp1ClassName, componentLargeBp2ClassName].join(' '))
	...
	test('Renders a component with a conditional variant repeatedly', () => {
		...
		expect(render(component, { size: { '@bp1': 'small', '@bp2': 'large' } }).props.className).toBe(`c-PJLV c-PJLV-fHtTAQ-size-small c-PJLV-XwbVw-size-large`)
		expect(toString()).toBe(
			`--sxs{--sxs:4 c-PJLV-fHtTAQ-size-small c-PJLV-XwbVw-size-large}@media{` + `@media (max-width: 767px){.c-PJLV-fHtTAQ-size-small{font-size:16px}}` + `@media (min-width: 768px){.c-PJLV-XwbVw-size-large{font-size:24px}}` + `}`,
		)
	})
