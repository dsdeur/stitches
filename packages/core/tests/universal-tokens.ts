...
import type { CSSValue } from '../src/types.ts'

/** Helper to access theme scale tokens in tests — tokens coerce to strings via toString() */
const scale = (theme: Record<string, unknown>, name: string) => theme[name] as Record<string, CSSValue>
...
describe('Tokens', () => {
	...
	test('Authors can use tokens from the globalCss theme object', () => {
		...
		globalCss({
			article: {
				marginLeft: scale(theme, 'space').sp1,
				marginTop: scale(theme, 'space').sp2,
			},
		})()
	...
	test('Authors can use tokens from a new theme object', () => {
		...
		globalCss({
			article: {
				marginLeft: scale(mytheme, 'space').sp1,
				marginTop: scale(mytheme, 'space').sp2,
			},
		})()
