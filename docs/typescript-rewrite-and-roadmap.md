...
| #1038 | 20 | Type-checking without `strict` takes minutes | **Not reproducible 2026-09-05.** `docs/bench/type-perf/` (40 deeply composed components) type-checks in 2.2 to 3.0s without `strict` and 2.3 to 2.9s with it on TypeScript 6, three warm runs each: indistinguishable. The report was against TS 4.6/4.7 in 2022. No change made; the `out` variance annotations suggested in the thread would be a speculative fix. Re-measure with the fixture if a consumer reports slowness. |
